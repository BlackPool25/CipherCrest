export async function fetchFlows() {
  try {
    ...
    // Array may be {flows:[] } shape from /report; normalize
    if (data && Array.isArray(data.flows) && data.flows.length > 0) return data.flows
    throw new Error('empty flows')
  } catch {
    // offline fallback: import fixtures directly (no CDN, bundled)
    try {
      ...
      // Vite JSON import may be {default: {...}} — unwrap
      const u = (m) => m.default || m
      return [u(f1), u(f6), u(f9)]
    } catch {
      // last resort inline fallback (system fonts only, no external)
      return [
        {
          flow_id: 'family-01',
          app_protocol: 'smtp',
