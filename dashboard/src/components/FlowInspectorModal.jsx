    ...
    >
      ...
      >
        ...
        <div style={{ padding: 24, overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {(activeTab === 'dual_ai' || typeof window !== 'undefined') && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {/* Dual Model Cards Side-by-Side */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(380px, 1fr))', gap: 16 }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 6, background: '#E7F5EC', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Cpu size={16} color={TOK.primary} />
                      </div>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 800, color: TOK.ink }}>Model 1: Risk Classifier</div>
                        <div style={{ fontSize: 10.5, color: TOK.inkMuted }}>Calibrated Gradient Boosted Classifier (Platt Scaled)</div>
                      </div>
                    </div>
                    <span style={{
                      background: sevColor(riskLevel),
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div style={{ background: '#FFFFFF', padding: '10px 12px', borderRadius: 8, border: `1px solid ${TOK.border}` }}>
                      <div style={{ fontSize: 10, color: TOK.inkMuted, textTransform: 'uppercase', fontWeight: 700 }}>Calibrated Probability</div>
                      <div className="tabular-nums" style={{ fontSize: 18, fontWeight: 800, color: TOK.ink, marginTop: 2 }}>
                        {calibratedProb.toFixed(3)}
                      </div>
                      <div style={{ fontSize: 10, color: calibratedProb >= 0.5 ? '#DC2626' : '#16A34A', fontWeight: 600 }}>
                        {calibratedProb >= 0.5 ? 'Exceeds 0.50 Threshold' : 'Below Risk Threshold'}
                      </div>
                    </div>

                    <div style={{ background: '#FFFFFF', padding: '10px 12px', borderRadius: 8, border: `1px solid ${TOK.border}` }}>
                      <div style={{ fontSize: 10, color: TOK.inkMuted, textTransform: 'uppercase', fontWeight: 700 }}>Decision Confidence</div>
                      <div className="tabular-nums" style={{ fontSize: 18, fontWeight: 800, color: TOK.primary, marginTop: 2 }}>
                        {(Math.max(calibratedProb, 1 - calibratedProb) * 100).toFixed(1)}%
                      </div>
                      <div style={{ fontSize: 10, color: TOK.inkMuted }}>
                        Brier: 0.117 • ECE: 0.21
                      </div>
                    </div>
                  </div>

                  <div style={{ fontSize: 11, fontWeight: 700, color: TOK.ink, marginTop: 4 }}>
                    Feature Attributions &amp; Decision Penalties:
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {featureWeights.slice(0, 4).map((fw, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#FFFFFF', borderRadius: 6, border: `1px solid ${TOK.border}`, fontSize: 11 }}>
                        <div>
                          <b style={{ color: TOK.ink }}>{fw.feature}:</b> <span style={{ color: TOK.inkMuted }}>{fw.value}</span>
                        </div>
                        <span style={{ fontWeight: 700, color: sevColor(fw.severity), fontFamily: TOK.fontMono }}>
                          {fw.impact}
                        </span>
                      </div>
                    ))}
                ...
                {/* Model 2: Isolation Forest & ECOD Anomaly Detector */}
                <div style={{
                  background: TOK.canvas,
                  border: `1px solid ${TOK.border}`,
                  borderRadius: 12,
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 10.5, color: TOK.inkMuted }}>ECOD + Isolation Forest Fingerprint Contrast</div>
                      </div>
                    </div>
                    <span style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div style={{ background: '#FFFFFF', padding: '10px 12px', borderRadius: 8, border: `1px solid ${TOK.border}` }}>
                      <div style={{ fontSize: 10, color: TOK.inkMuted, textTransform: 'uppercase', fontWeight: 700 }}>ECOD Anomaly Score</div>
                      <div className="tabular-nums" style={{ fontSize: 18, fontWeight: 800, color: isAnomaly ? '#DC2626' : TOK.ink, marginTop: 2 }}>
                        {anomalyScore.toFixed(2)}
                      </div>
                      <div style={{ fontSize: 10, color: TOK.inkMuted }}>
                        Threshold: 16.50 (Baseline: 14.90)
                      </div>
                    </div>

                    <div style={{ background: '#FFFFFF', padding: '10px 12px', borderRadius: 8, border: `1px solid ${TOK.border}` }}>
                      <div style={{ fontSize: 10, color: TOK.inkMuted, textTransform: 'uppercase', fontWeight: 700 }}>JA4 Rarity Percentile</div>
                      <div className="tabular-nums" style={{ fontSize: 18, fontWeight: 800, color: '#2563EB', marginTop: 2 }}>
                        0.926
                      </div>
                      <div style={{ fontSize: 10, color: TOK.inkMuted }}>
                        GREASE 16 Filtered
                  ...
                  <div style={{ fontSize: 11, fontWeight: 700, color: TOK.ink, marginTop: 4 }}>
                    Fingerprint Diagnostics &amp; Deviation Rationale:
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <div style={{ padding: '8px 10px', background: '#FFFFFF', borderRadius: 6, border: `1px solid ${TOK.border}`, fontSize: 11 }}>
                      <b style={{ color: TOK.ink }}>JA4 Fingerprint:</b>
                      <div className="mono" style={{ fontFamily: TOK.fontMono, color: TOK.primary, fontSize: 10.5, marginTop: 2, wordBreak: 'break-all' }}>
                        {flow.tls?.ja4 || 't13d0300_000000000000_000000000000'}
                      </div>
                    </div>
                    <div style={{ padding: '8px 10px', background: '#FFFFFF', borderRadius: 6, border: `1px solid ${TOK.border}`, fontSize: 11 }}>
                      <b style={{ color: TOK.ink }}>Deviation Verdict:</b> {isAnomaly ? 'Rare cipher suite negotiation order and non-standard extension sequence detected.' : 'Handshake extensions align with standard Postfix / Exim client profiles.'}
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'policy' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{
                background: sevBg(riskLevel),
                border: `1.5px solid ${sevColor(riskLevel)}40`,
                borderRadius: 12,
                padding: '16px 20px',
              }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: sevColor(riskLevel) }}>
                  Gateway Policy Action: {riskLevel === 'Critical' ? 'QUARANTINE / BLOCK' : riskLevel === 'High' ? 'FLAG & AUDIT' : 'ALLOW TRAFFIC'}
                </div>
                <div style={{ fontSize: 12, color: TOK.ink, marginTop: 6, lineHeight: 1.5 }}>
                  Based on calibrated risk score ({riskScore}/100) and anomaly detection telemetry, this flow requires {riskLevel === 'Critical' ? 'immediate cryptographic upgrade or connection termination.' : 'continuous verification against enterprise baselines.'}
                </div>
              </div>
            </div>
          )}
