...
export default function Reports() {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      ...
      for (let p = 0; p < totalPages; p++) {
        ...
        pdf.text(
          `Sandesh Kavach • Page ${p + 1} of ${totalPages} • Cryptographic Posture Assurance`,
          margin,
          pageH - 4
        )
      ...
      pdf.save(`Sandesh-Kavach-${reportType.toUpperCase()}-Report-${new Date().toISOString().slice(0, 10)}.pdf`)
  ...
  const handleExportJSON = useCallback(() => {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `sandesh-kavach-${reportType}-report-${new Date().toISOString().slice(0, 10)}.json`
    document.body.appendChild(a)
    a.click()
    a.remove()
  ...
  const handleCopyMarkdown = useCallback(() => {
    const md = `
# Sandesh Kavach — Executive Posture Report
...
## Executive Summary
Sandesh Kavach conducted passive cryptographic and transport layer posture assessments across ${flows.length} enterprise email flows (Ports 25, 587, 143, 110, 993). Overall fleet encryption posture is scored at **${avgPosture}/100**. ${urgentThreats.length > 0 ? `**Action Required:** ${urgentThreats.length} flows exhibit severe vulnerabilities requiring immediate remediation.` : 'All monitored traffic satisfies RFC 8996 and NIST SP 800-52r2 encryption standards.'}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, width: '100%' }}>
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 800, color: TOK.ink, letterSpacing: -0.4 }}>
                Sandesh Kavach <span style={{ color: TOK.primary }}>Cryptographic Armor</span>
              </div>
        ...
        <div style={{
          ...
          <div>
            Generated by <b>Sandesh Kavach</b> • Automated Passive Telemetry Evaluator
          </div>
          <div>
