    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: '#151B1E', letterSpacing: -0.3 }}>
      ...
      <div style={{ overflowX: 'auto', border: '1px solid #E7EAEC', borderRadius: 10 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr style={{ background: '#F6F8F7', color: '#6B7280', textAlign: 'left', borderBottom: '1px solid #E7EAEC' }}>
          ...
          <tbody>
            {PORTS.map(r => (
              <tr key={r.port} style={{ borderBottom: '1px solid #E7EAEC', color: '#151B1E' }}>
                <td style={{ padding: '10px 14px', fontWeight: 700 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Server size={14} color="#1F7A4D" />
                    <span>{r.label}</span>
                  </div>
                </td>
                ...
                <td style={{ padding: '10px 14px', fontFamily: 'monospace', color: '#1F7A4D', fontWeight: 700 }}>
                ...
                <td style={{ padding: '10px 14px' }}>
                  <span style={{ background: '#E7F5EC', color: '#1F7A4D', padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 700 }}>
                    {r.compliance}
                  </span>
                </td>
                <td style={{ padding: '10px 14px', color: '#6B7280', fontSize: 11 }}>
                ...
                <td style={{ padding: '10px 14px', color: '#6B7280', fontSize: 11, fontStyle: 'italic' }}>
                  {r.diff}
                </td>
              </tr>
            ))}
      ...
      <div style={{ border: '1px solid #E7EAEC', borderRadius: 10, overflow: 'hidden' }}>
        <button
          type="button"
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <FileText size={15} color="#1F7A4D" />
            <span>RFC Specification Annex &amp; Verification Matrix (R1–R8)</span>
            <span style={{ fontSize: 11, fontWeight: 500, color: '#6B7280' }}>• 14/20 Verified REAL + 3 Info Checks</span>
          </div>
          {showAnnex ? <ChevronUp size={16} color="#6B7280" /> : <ChevronDown size={16} color="#6B7280" />}
        </button>

        {showAnnex && (
          <div style={{ overflowX: 'auto', maxHeight: 260, overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
              <thead style={{ position: 'sticky', top: 0, background: '#FFFFFF', zIndex: 2 }}>
                <tr style={{ color: '#6B7280', textAlign: 'left', borderBottom: '1px solid #E7EAEC' }}>
              ...
              <tbody>
                {R8.map(r => (
                  <tr key={r.id} style={{ borderBottom: '1px solid #E7EAEC', color: '#151B1E' }}>
                    <td style={{ padding: '8px 12px', fontWeight: 800, color: '#1F7A4D' }}>{r.id}</td>
                    <td style={{ padding: '8px 12px', fontWeight: 600 }}>{r.limit}</td>
                    <td style={{ padding: '8px 12px', color: '#6B7280' }}>{r.coverage}</td>
                    <td style={{ padding: '8px 12px', color: '#6B7280' }}>{r.mit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
