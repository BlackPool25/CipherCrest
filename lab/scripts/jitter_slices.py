...
def make_pcap(fam:str,idx:int,seed:int)->pathlib.Path:
    ...
    hello+=f" CIPHER={cfg['cipher']} SIGALG=sha384 EXPIRY_JITTER={delta:+d}d JA4_RARITY={rarity} JA4={ja4_key or 'unknown'} GREASE=0x0a0a".encode()
    ...
    OUT_DIR.mkdir(parents=True,exist_ok=True);out=OUT_DIR/f"family-{fam}-jitter-01.pcap"
    wrpcap(str(out),pkts);print(f"Wrote {out} ({len(pkts)} pkts, ja4_rarity={rarity})")
    REASM_DIR.mkdir(parents=True,exist_ok=True);(REASM_DIR/f"family-{fam}-jitter-01.bin").write_bytes(hello[:120])
    return out

def update_manifest(families:list[str])->None:
    ...
    for k,ent in data.items():
        if "environment_id" not in ent:
            ent["environment_id"]=f"{k}__postfix3.9_loss0";ent["capture_epoch"]=CAPTURE_EPOCH;ent["client"]="sender";ent["docker_image_sha256"]=DOCKER_SHA;ent["tshark_version"]=TSHARK_VER;ent["source_id"]=str(uuid.uuid4())
    MANIFEST.write_text(json.dumps(data,indent=2)+"\n",encoding="utf-8");print(f"Updated {MANIFEST}")

def update_ledger(families:list[str])->None:
    sha_map={}
    for fam in families:
        p=OUT_DIR/f"family-{fam}-jitter-01.pcap"
        if p.exists():sha_map[fam]=hashlib.sha256(p.read_bytes()).hexdigest()
    ...
    if "environment_id" not in text:
        ...
        for l in lines:
            ...
            elif l.startswith("| 01 |") or l.startswith("| 02 |") or l.startswith("| 03 |") or l.startswith("| 04 |") or l.startswith("| 05 |") or l.startswith("| 06 |") or l.startswith("| 07 |") or l.startswith("| 08 |") or l.startswith("| 09 |") or l.startswith("| 10 |"):
    ...
    for fam in families:
        jid=f"{fam}-jitter-01"
        if jid in text:continue
        cfg=FAMILY_CFG[fam];sha=sha_map.get(fam,"pending");env=f"family-{fam}__postfix3.9_loss0";src=str(uuid.uuid4())[:8]
        text+=f"| {jid} | {env} | {CAPTURE_EPOCH} | {sha} | {cfg['starttls']} | {cfg['cipher']} (+GREASE sha384) | {cfg['cert']} | PASS | 1.0 | {src} | 1 | # jitter slice cipher-shuffle GREASE 0x0a0a sigalg sha384 expiry +-5d ja4_rarity sampled\n"
...
def main()->None:
    ...
    update_manifest(fams);update_ledger(fams)
