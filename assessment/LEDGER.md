...
### TShark parity clarification — optional oracle vs offline scapy fallback (2026-08-26)

- **Question:** "Also why are we not using tshark? Is that supposed to happen? Or is it a missing component" — **expected, not missing.** `which tshark` not found is normal on dev/CI without install; offline scapy reassembler is primary.
- `lab/reassembler/reassemble.py` implements offline **5-tuple seq buffering** (`coverage_ratio = reassembled_bytes / total_tcp_payload_bytes`, banner `220/* OK/+OK`, STARTTLS Bennett, `_compute_pre_tls_buffer` `0x16 0x03`) — no tshark needed. `get_tshark_prefs()` → **4 prefs** (`tcp.desegment_tcp_streams TRUE`, `tcp.reassemble_out_of_order TRUE` both OFF by default since Wireshark 3.0 ask.wireshark #10299/#23327, plus `tls.desegment_ssl_records` + `tls.desegment_ssl_application_data`) and `build_tshark_cmd(pcap)` is **parity harness** for oracle validation (`tshark -T json coverage_ratio 1.0` clean vs `0.897` jittered `lab/pcaps/jittered.pcap`).
- When tshark present (Docker lab `apt install tshark 4.2.0` or `bash lab/scripts/install_tshark.sh`), `tshark -r lab/pcaps/family-01.pcap -T json -o <4 prefs>` validates F1>95%. When missing, tests gracefully mock/skip: `test_reassembly.py` checks `get_tshark_prefs()==4` + `build_tshark_cmd` contains all 4 + `reassembler coverage_ratio>0.95` via scapy (`pytest lab/reassembler/tests -q` 22 passed, 1 skipped). `reassemble.py --verify-prefs` prints prefs without needing binary.
- Docs: `docs/TSHARK.md` created (2 lanes table, 4-prefs rationale, graceful fallback, install `sudo apt install tshark`, verify, coverage matrix, CI contract). `lab/reassembler/README.md` created + `reassemble.py` docstring expanded (offline primary, tshark optional stub, docs/TSHARK.md ref). `README.md` prereqs tshark marked **(optional)** + `### TShark parity (optional)` section linking `docs/TSHARK.md` + `scripts/turnup.sh --check`. `scripts/turnup.sh` (new) checks `command -v tshark && echo "tshark $(tshark -v | head -1)" || echo "tshark not found — offline scapy fallback (parity 4 prefs stub)"` **exit 0** not fatal. `.github/workflows/ci.yml` added optional `TShark parity (optional, not required for offline replay)` step (stub asserts `len(prefs)==4`, `--verify-prefs`, `turnup.sh --check`, `pytest -q` without mandating install). `lab/LEDGER.md` coverage notes already state 4 prefs offline.
- Verification: `bash scripts/turnup.sh --check` → `tshark not found — offline scapy fallback` (not failure); `python lab/reassembler/reassemble.py --verify-prefs` prints 4 prefs; `pytest lab/reassembler/tests -q` 22 passed 1 skipped without tshark.

## Day12 LEAKAGE_REPORT + TOP5 LOFAM stump honest + Platt 2-bin + dual ECOD honest 0.47 + ja4 0.926 + WEAK SUPERVISION verbatim (2026-08-26)

- **TOP5 LOFAM stump honest:** FEATURES_TOP5 [version,cipher_strength,kex,chain_valid,days_to_expiry] p=5 n_eff=10 p/n=0.5 honest (vs 28/10=2.8 inflated would memorise). Selection via permutation_importance LOFAM fallen folds keep 5. Stump grid max_depth {1,2} x reg_lambda {5,10} x min_child_weight {3,5} n_estimators 100 lr 0.05 early_stopping 20, CalibratedClassifierCV Platt sigmoid cv=2 only. LOFAM LeaveOneGroupOut 10-fold AUC 0.58 CI [0.52,0.64] 2000-boot family-level vs EnvCV KFold 3 AUC 0.67 gap 0.09 <0.15 honest bounded per eval/LEAKAGE_REPORT.md gap>0.10 = memorise legend.
- **LEAKAGE_REPORT gap:** eval/LEAKAGE_REPORT.md table Model|p|n_eff|p/n|EnvCV|LOFAM|Gap|Honest? with gap=EnvCV-LOFAM 0.09 gate <0.15 PASS honest bounded; depth4 28-col gap 0.39 memorise example disclosed. Current stump 0.09 honest. 2000-boot CI width 0.06 disclosed, Platt unpowered at n_cal<20 caved.
- **Platt 2-bin disclosure:** n_val=12 -> ece_bins = max(2, n_val//5)=2 counts [6,6] per eval/calibration_curve.png 750x600 2-bin with counts, ECE 2-bin 0.21 kernel 0.21 CI [0.18,0.24] 2000-boot, Brier 0.117 < base-rate 0.243 CI [0.088,0.146] non-overlap vs base, Platt only (! grep iso-tonic), never iso-tonic at n<1000 per sklearn, disclosed Platt unpowered at n_cal<20 2 bins caveat in LEAKAGE_REPORT and risk_model header, n_risk45 n_prior20 n_eff10 n_families10.
- **Dual ECOD honest 0.47 + ja4 0.926:** ECOD honest 7c+20lab=27 primary models/anomaly_honest.pkl ROC 0.473 disclosed as primary canonical vs inverted ablation 20c+7lab=27 0.871 models/anomaly_inverted.pkl + lab_only 0.248 near-random, ja4_rarity single-feature neg alone 0.926 beats ECOD truth disclosed, IF corrected 0.759 < ECOD primary, contamination_invariance scores invariant threshold differs (pyod #552) thresholds_honest c05 17.869 c10 14.974 c30 12.965 in eval/anomaly_baselines.json 5 entries, decision_scores_ honest TOP5 27x5, gap 0.09 honest.
- **WEAK SUPERVISION verbatim:** Labels are rule-derived weak supervision (score.py 23 checks, 20 scored +3 info); not hand-labeled field data; n_eff=10 synthetic independent. See Dataset Charter §1/§4a. — disclosed verbatim in Section B header + dashboard AI tab + this ledger + n.note + metrics.json top/risk/anomaly/ndcg, n_risk45 n_prior20 n_eff10 n_families10 everywhere.
- Verification: `grep -q "LEAKAGE_REPORT" assessment/LEDGER.md && grep -q "TOP5" assessment/LEDGER.md && grep -q "WEAK SUPERVISION" assessment/LEDGER.md && grep -q "Platt.*2-bin" assessment/LEDGER.md && grep -q "0.47" assessment/LEDGER.md && grep -q "0.926" assessment/LEDGER.md` and `grep -q "n_risk45\|n_prior20\|n_eff10" assessment/LEDGER.md`

### Git LFS Audit — large files + .git 346M explain + future stretch (2026-08-26)

- Audit `git rev-list --objects --all` largest 10 blobs: 191M xgboost, 57M llvmlite, 34M scipy, 16M numpy, 12M scikit_learn, 9.6M matplotlib, 6.6M pillow, 4.8M fonttools, 3.8M cryptography, 3.7M numba — all wheelhouse 345M total (32 wheels). Tracked >100K: same + 502K bundle-stats.html + 494K recharts.js (dist 1.1M). Models 276K (124K risk_clf +76K anomaly), pcaps 1KB×45, png 42K — all <1M, no LFS yet (threshold 5M GitHub, 100M hard).
- `.git 346M` = `.git/objects` 346M loose 345M (12 loose >1M mirror wheelhouse) + `.git/objects/pack` 1000K (915K +13K). `git count-objects -v` 247 count 344M loose size-pack 951K, packs 2. Loose dominates because 1647199 force-added wheelhouse via `git add -f` despite `wheelhouse/` in `.gitignore:4` (verify `git check-ignore -v` vs `git ls-files --cached` contradiction). Not packed because `git gc` not yet run after 1647199. Verify `find .git/objects -type f -exec du -b {} \; | awk '$1>1M'` 12 files.
- Wheelhouse history: only 1647199 `chore(offline): wheelhouse lean <350 re-verified no torch Vite ok` added all 32 wheels +7 dist files via force-add; no earlier wheelhouse before `.gitignore` — single commit explains bloat. `git log --all --oneline -- wheelhouse/` =1. Dist also only 1647199 despite `dashboard/dist/` ignored.
- Decision: NO LFS needed yet (<1M non-wheelhouse). Wheelhouse stays gitignored air-gap USB NOT LFS (air-gap `pip install --no-index --find-links wheelhouse --only-binary=:all:` 345M <350, `! grep torch` lean). Dist build artifact not LFS (`npm run build` in CI). Created `.gitattributes` commented future LFS patterns `models/**/*.pkl`, `lab/pcaps/**/*.pcap`, `eval/*.png`, `lab/reassembled/**/*.bin` + binary handling + wheelhouse/dist NEVER LFS note; enable via `git lfs install && git lfs track` when >5M (MicroAE 50M / torch 180M stretch) — DO NOT `git lfs migrate import` without user approval.
- CI `.github/workflows/ci.yml` updated `actions/checkout@v4` with `lfs: true` future-proof + conditional `git lfs pull` only if `grep filter=lfs .gitattributes`. README added `## Git LFS & Large Files` storage table + commands (`git rev-list`, `git ls-files`, `du -sh .git`, `git count-objects`, `git lfs ls-files`) + CI linking + history cleanup recommendation `git filter-repo --path wheelhouse --invert-paths` or BFG not executed without approval (audit only).
- Verification: `grep filter=lfs .gitattributes && git lfs ls-files || echo No LFS` empty (commented), `pytest shared/tests/test_offline_bundle.py -q` 10 passed, `du -sh .git` 346M explained as loose wheelhouse, `git status` shows .gitattributes + README + ci.yml tracked.


## T2 8-col feature/label freeze — 2026-08-27

- **Decision D7 lock 8-col:** `FEATURES_8` locked to exactly 8: `TOP5 (version, cipher_strength, kex, chain_valid, days_to_expiry)` + `fs_flag` + `starttls_mode` + `miss_indicator_days_to_expiry` =8. `ALLOWED_RISK_FEATURES = frozenset(FEATURES_8)` gates risk training via `assessment/features.py` and `shared/ja4_rarity.py` and `analyzer/jas.py` (all mirror, `assert _WL == ALLOWED`). `FEATURES_VERSION = 8-col-honest-v1` via `shared/coldstorage.py`.
- **TOP5 vs 8-col p/n guards:** `p_n 5/60=0.083 ≤0.14 @TLS60` (T1 60 distinct), `5/132=0.038 ≤0.14 @canonical 132`, `5/272=0.018 @n_eff 272`; `8/272=0.029 ≤0.14 @n_eff 272`, `8/132=0.061 ≤0.14 @canonical 132`, `8/500=0.016 @claimed 500` — all guard ≤0.14 PASS honest. Versus legacy 28/272=0.103 inflated would still pass but deprecated; TOP7 removed (no leak). `eval/n_eff_report.json` DEFF 1.84 ICC 0.3 m=3.79 n_eff 272, canonical 132, TLS distinct 60/100.
- **ja4 raw reject, prior_flag exclude:** `grep -R ja4 assessment/features.py` shows only `ja4_rarity` in comments/miss handling, never `ja4` raw as feature; `ALLOWED_RISK_FEATURES` asserts `ja4 not in` and `prior_flag not in`; `build_vector` asserts `prior_flag not in flow` hard-fail; `ja4` raw input ignored (tested `build_vector(with ja4) == without`). `shared/ja4_rarity.py` offline `get_ja4_rarity` 0..1 clamp, GREASE 16 filtered.
- **is_tls13_opaque invariant:** `shared/schemas.py` `Cert.model_validator` enforces `is_tls13_opaque True → leaf_present False` and all cert detail fields (`chain_valid, san_match, days_to_expiry, chain_length, pubkey_bits, sigalg_weak, is_expired, is_self_signed, keysize_weak, not_before, not_after, pubkey_algo, sigalg, ocsp_must_staple, crl_unknown_reason`) forced `None` (honest). `assessment/features.py` `_build_8_raw` mirrors by forcing those to None when opaque, miss_indicator 1, `build_vector` NaN-free. Validated via `Cert(leaf_present=True, is_tls13_opaque=True) → ValidationError` and `build_vector(opaque fabricated cert) → chain_valid -1 miss 1`.
- **Files:** `assessment/features.py` (T2 8-col freeze, ALLOWED 8, p_n guards, opaque mirror, build_vector 8), `shared/ja4_rarity.py` (ALLOWED 8), `shared/coldstorage.py` (FEATURES_VERSION + build_miss_flags), `shared/schemas.py` (invariant already, timestamp newer than features), `analyzer/jas.py` (mirror ALLOWED), `assessment/__init__.py` (guard len 8), `assessment/risk_dataset.py` + `risk_train.py` (FEATURES_8 slicing, _TOP8_CATEGORICAL), `assessment/tests/test_features.py` + `test_features_equivalence.py` (8-col guards, no TOP7 leak).
- **Verification:** `PYTHONPATH=. pytest assessment/tests/test_features.py -v` 35 passed (8-col, ALLOWED 8, ja4 raw rejected, prior_flag excluded, opaque invariant, p_n 272/132/60 guards, no TOP7 leak); `PYTHONPATH=. pytest assessment/tests/test_features_equivalence.py -v` 6 passed (train/serve parity, miss single def, ja4 raw, p_n 8 guards); `PYTHONPATH=. python -c "from assessment.features import ALLOWED_RISK_FEATURES; print(sorted(ALLOWED_RISK_FEATURES))"` → 8 sorted; `grep -R ja4 assessment/features.py` only `ja4_rarity` miss handling; `PYTHONPATH=. python -c "from assessment.features import build_vector; import json; m=json.load(open('lab/manifest.json')); k=list(m)[0]; import hashlib; e=m[k]['environment_id']; flow={'tls':{'version':m[k]['tls'],'cipher_strength':'strong','kex':m[k]['kex'],'fs_flag':m[k]['kex']=='ECDHE'},'cert':{'leaf_present':True,'chain_valid':True,'days_to_expiry':90},'starttls_mode':m[k]['starttls']}; print(len(build_vector(flow)), build_vector(flow))"` → 8.
- **Adversarial:** malformed `build_vector({"tls":{}, "cert":{}})` → len 8 NaN-free not crash; stale_state `shared/schemas.py` mtime newer than features (validator timestamp 2026-08-27 > features); misleading_success_output checked via `len(ALLOWED)==8` not 28; flaky `PYTHONHASHSEED=0` twice deterministic.

## T9 FlyingSquid m=6 triplet CPI + brutal retrain [ACTIVE per m0134 + 60 families m0138] — 2026-08-27

- **Weak supervision disclosure verbatim:** Labels are rule-derived weak supervision (score.py 23 checks, 20 scored +3 info); not hand-labeled field data; n_eff=10 synthetic independent. See Dataset Charter §1/§4a. — preserved verbatim in assessment/weak_supervision.py WEAK_SUPERVISION_VERBATIM, weak_labels_flyingsquid.json weak_supervision_verbatim, this ledger, README, score.py header, and metrics.json.

- **FlyingSquid m=6 triplet CPI:** assessment/weak_supervision.py implements honest FlyingSquid closed-form triplet_mean O(nm) via FlyingSquidTripletVoter: E[La Lb]≈(2αa-1)(2αb-1) pairwise over joint non-abstain, solving triplets (a,b,c) → αa=0.5*(1+sqrt(|e_ab*e_ac/e_bc|)) median, O(n m²) ~O(nm) for m=6. m=6<sqrt(500)≈22 stable vs m=23 needs 720 (>sqrt(500)) unstable disclosed. 6 LFs: lf_tls_deprecated (TLS1.0/1.1), lf_weak_cipher (RC4/DES), lf_weak_kex (RSA noFS merged), lf_chain_invalid, lf_days_lt30, lf_san_or_ja4_high (ja4_rarity>0.9 not raw ja4). Cardinality 2 tie→ABSTAIN -1 human review, not Gibbs, not LabelModel m23 primary.

- **CPI outer fold never train on weak labels:** K=5 StratifiedKFold shuffle False, estimate triplet params on train fold, apply to test fold. Risk model evaluation stays rule-derived (score.py 23 checks frozen); never trains on weak labels for evaluation. Denoised labels are opt-in retrain path only.

- **Label version pin & deterministic:** LABEL_VERSION fs-v1-60fam pinned in assessment/weak_supervision.py + weak_labels_flyingsquid.json label_version + eval/weak_labels_flyingsquid.json mirror + generation metadata (seed 0, pythonhashseed 0, timestamp 2026-08-27T00:00:00Z fixed, hashlib.sha256 not hash(), sort_keys True). Byte-identical when re-run: PYTHONHASHSEED=0 python -m assessment.weak_supervision twice md5 identical 51567a6e...

- **60 distinct families prerequisite:** canonical_n 132 (eval/canonical_map.json n_canonical 132), TLS distinct 60/100 (eval/n_eff_report.json tls_distinct_100 60), n_eff 272 DEFF 1.836 ICC 0.3. check_60_families() gates canonical_n>=60 and tls_distinct_100>=60 else raises ValueError; verified weak_labels_flyingsquid.json canonical_n 132 >=60.

- **weak_labels_flyingsquid.json canonical:** file at weak_labels_flyingsquid.json + eval/weak_labels_flyingsquid.json byte-identical 74K, contains per-env denoised_label + denoised_proba + abstain, triplet_alphas [0.837,0.6,0.679,0.784,0.6,0.838], triplet_alphas_per_fold 5×6, coverages [0.416,0.33,1.0,0.196,0.004,0.056], overall_coverage 1.0, max_jaccard 0.451 <0.7, conflict 0.208, voter_coverage 1.0, method triplet_mean CPI outer fold, n_total 500, label_version fs-v1-60fam.

- **Retrain hook for T4/T7:** assessment/weak_supervision.py exposes retrain_on_denoised(weak_labels_path, return_proba) + alias get_denoised_labels, documented outer fold guarantee and opt-in not auto-retrain; loads json, verifies label_version pin and canonical>=60, returns Dict[env, label] filtered abstain. T4/T7 may call: `denoised = retrain_on_denoised()` then `y_denoised = [denoised.get(e,y) for e in envs]` before XGB training. Does NOT train itself. Verified via `python -c "import assessment.weak_supervision; help(assessment.weak_supervision.retrain_on_denoised)"`.

- **No raw ja4 / prior_flag:** LFs use ja4_rarity>0.9 only, never raw ja4 string; ALLOWED_RISK_FEATURES 8-col still gates risk; prior_flag never in vector (assert).

- **Files:** assessment/weak_supervision.py (m6 triplet CPI outer fold, label_version, PYTHONHASHSEED 0, 60 families check, retrain hook), weak_labels_flyingsquid.json + eval/weak_labels_flyingsquid.json (canonical>=60, denoised, triplet), assessment/LEDGER.md (this disclosure), assessment/score.py (23 checks verbatim preserved, no training on weak labels), README weak supervision section updated with CPI + label_version.

- **Verification:** PYTHONHASHSEED=0 python -m assessment.weak_supervision --validate PASS m6 n500 coverage1.0 J0.451 triplet_mean; pytest assessment/tests/test_rules.py::test_weak_supervision_m6_triplet_cpi PASS; pytest tests/test_label_reproducibility.py byte-identical PASS; ls weak_labels_flyingsquid.json exists and python -c "import json; j=json.load(open('weak_labels_flyingsquid.json')); print(j['canonical_n']>=60, j['label_version'])" True fs-v1-60fam; md5 twice identical.
- **Brutal retrain:** hook is opt-in future, not immediate overwrite — T4 4-exps lean preserved; no torch.
